...
export const setSessionCookie = (user: AdminUser) => {
  if (typeof window !== 'undefined') {
    const expires = new Date();
    expires.setTime(expires.getTime() + (7 * 24 * 60 * 60 * 1000));
    document.cookie = `nbp_admin_session=${encodeURIComponent(JSON.stringify(user))}; expires=${expires.toUTCString()}; path=/; SameSite=Lax; Secure`;
  }
};

export const clearSessionCookie = () => {
  if (typeof window !== 'undefined') {
    document.cookie = 'nbp_admin_session=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT; SameSite=Lax; Secure';
  }
};
...
export const authService = {
  ...
  async isApprovedAdmin(email: string): Promise<boolean> {
    if (isMockSupabase()) {
      ...
      return ['techinventiveworks@gmail.com', 'bpath2026@gmail.com', 'admin@nbp.com', 'guest@nbp.com'].includes(email.toLowerCase());
    }
  ...
  async signInWithPassword(email: string, password: string): Promise<{ user: AdminUser | null; error: Error | null }> {
    ...
    if (isMockSupabase()) {
      ...
      const mockUser: AdminUser = {
        ...
        email: cleanEmail,
        role: (cleanEmail === 'techinventiveworks@gmail.com' || cleanEmail === 'bpath2026@gmail.com') ? 'super_admin' : 'admin',
