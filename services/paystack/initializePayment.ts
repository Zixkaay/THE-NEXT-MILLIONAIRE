...
export async function initializePaystackPayment(
  ...
  if (isMockSupabase() && !process.env.NEXT_PUBLIC_SUPABASE_URL) {
    ...
    // In a real Next.js app on the server, we don't have window.
    // We should ideally use an environment variable for the base URL.
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
    
    return {
      success: true,
      reference: mockRef,
      authorization_url: `${baseUrl}/api/payment/simulate?ref=${mockRef}&email=${encodeURIComponent(email)}&amountGHC=${amountInGHC}&metadata=${encodeURIComponent(JSON.stringify(metadata))}`
    };
  }
  ...
  try {
    ...
    return {
      ...
      authorization_url: `${baseUrl}/success?ref=NBP-MOCK-` + Math.floor(Math.random() * 100000000),
