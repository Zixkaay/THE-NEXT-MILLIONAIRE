...
import { MapPin, Phone, Mail, Globe, Shield, Target, Zap, TrendingUp, Users, Presentation, Award, Search, MessageCircle, Code, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="bg-bg min-h-screen pt-40 md:pt-48 pb-16 animate-in fade-in duration-500">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        {/* CEO Message Section */}
        <section className="mb-32 bg-[#0a0a0a] border border-[#262626] rounded-[3rem] p-12 md:p-20 relative">
          {/* Inner background container to handle rounded clipping for the gradient */}
          <div className="absolute inset-0 rounded-[3rem] overflow-hidden pointer-events-none">
            <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-[#d4af37]/5 to-transparent" />
          </div>

          {/* Founder Image - Desktop Overlapping Card */}
          <div className="hidden lg:block absolute -top-16 right-0 w-[300px] xl:w-[360px] bottom-0 z-20 group">
            <div className="relative w-full h-full">
              {/* Gold Glow behind the card */}
              <div className="absolute top-4 right-4 bottom-4 left-4 bg-gradient-to-t from-[#d4af37]/0 to-[#d4af37]/15 rounded-[2.5rem] blur-2xl opacity-75 group-hover:opacity-100 transition-all duration-500 pointer-events-none" />
              
              {/* Elegant Framed Image Card with nice margins inside parent */}
              <div className="absolute top-4 right-4 bottom-4 left-4 bg-[#121212] border border-[#d4af37]/40 rounded-[2.5rem] overflow-hidden shadow-[0_30px_70px_rgba(0,0,0,0.9)] transition-all duration-700 ease-out group-hover:border-[#d4af37] group-hover:scale-[1.02]">
                <img 
                  src="https://res.cloudinary.com/dnhz6xwjz/image/upload/v1782926762/Founder._Darlyn_Nana_Kwasi._ltpgqv.jpg"
                  alt="Darlyn Nana Kwasi"
                  className="w-full h-full object-cover object-top transition-all duration-700 ease-out group-hover:scale-105"
                  referrerPolicy="no-referrer"
                />
                {/* Elegant overlay gradient at bottom of image */}
                <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/60 to-transparent pointer-events-none" />
              </div>
            </div>
          </div>

          <div className="lg:max-w-[55%] xl:max-w-[60%] relative z-10">
            <h2 className="text-[#d4af37] font-bold text-sm tracking-[0.4em] uppercase mb-12">CEO / Founder Message</h2>
            
            {/* Mobile/Tablet Founder Image - Inline */}
            <div className="block lg:hidden w-full max-w-[280px] mb-8 relative group">
              <div className="absolute -inset-2 bg-gradient-to-r from-[#d4af37]/0 to-[#d4af37]/10 rounded-3xl blur-xl pointer-events-none" />
              <div className="relative bg-[#121212] border border-[#d4af37]/40 rounded-3xl overflow-hidden shadow-2xl">
                <img 
                  src="https://res.cloudinary.com/dnhz6xwjz/image/upload/v1782926762/Founder._Darlyn_Nana_Kwasi._ltpgqv.jpg"
                  alt="Darlyn Nana Kwasi"
            ...
            <div className="pt-8 border-t border-[#262626]">
              <p className="text-white font-black uppercase tracking-[0.2em] mb-1">DARLYN NANA KWASI</p>
              <p className="text-[#d4af37] font-serif italic">Founder & CEO, Billionaire’s Path</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-32">
          <div className="bg-white text-black rounded-[3rem] p-12 md:p-20 flex flex-col items-center text-center">
             ...
             {/* Managing Director Portrait Centerpiece */}
             <div className="mb-12 relative w-full max-w-[320px] aspect-[3/4] group mx-auto">
               {/* Soft Gold/Dark Shadow under/behind */}
               <div className="absolute -inset-1.5 bg-gradient-to-t from-[#d4af37]/15 to-black/10 rounded-3xl blur-xl opacity-80 group-hover:opacity-100 transition-all duration-500 pointer-events-none" />
               
               {/* Elegant Framed Image Card */}
               <div className="relative w-full h-full bg-[#121212] border-2 border-black/10 group-hover:border-[#d4af37] rounded-3xl overflow-hidden shadow-2xl transition-all duration-500 transform group-hover:scale-[1.02]">
                 <img 
                   src="https://res.cloudinary.com/dnhz6xwjz/image/upload/v1782926762/MANAGING_DIRECTOR_Dr._Kwaku_Nti._tpl7cv.jpg"
                   alt="Dr. Kwaku Nti"
                   className="w-full h-full object-cover object-top transition-all duration-700 ease-out group-hover:scale-105"
                   referrerPolicy="no-referrer"
                 />
                 ...
                 {/* Position Details overlay inside the image at the bottom */}
                 <div className="absolute bottom-0 inset-x-0 p-6 text-center z-10">
                   <h3 className="text-white text-xl font-black uppercase tracking-wider mb-1">Dr. Kwaku Nti</h3>
                   <p className="text-[#d4af37] text-xs font-bold uppercase tracking-[0.2em]">Managing Director</p>
                 </div>
               </div>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full text-left">
               {[
                 ...
                 <div key={i} className="border border-black/10 p-6 rounded-2xl flex items-center gap-4">
                   <div className="w-5 h-5 flex items-center justify-center text-black shrink-0 font-bold">✓</div>
        ...
        <section id="contact-section" className="mb-32 grid grid-cols-1 md:grid-cols-2 gap-12 scroll-mt-32">
          ...
          <div className="bg-[#d4af37] p-12 rounded-[2.5rem] flex flex-col items-center text-center justify-center shadow-[0_30px_60px_rgba(212,175,55,0.2)]">
            ...
            <a href="/register" className="bg-black text-white font-black uppercase text-xs tracking-[0.3em] py-5 px-10 rounded-full hover:scale-105 transition-transform flex items-center gap-3">
