...
import { getFormFieldsAction } from '@/features/form-builder/actions';
...
export function RegistrationForm() {
  ...
  const videoUploadVisible = settings ? settings.video_upload_visible : config.video_upload_visible;
  const videoRequired = settings ? settings.video_required : config.video_required;
  ...
  useEffect(() => {
    async function loadDynamicSteps() {
      try {
        const dbSteps = await getFormFieldsAction(false); // Fetch enabled fields only
        const filtered = dbSteps.map(step => {
          return {
            ...step,
        ...
        console.error('[RegistrationForm] Error loading dynamic schema:', err);
  ...
  // Photo and Video independent upload states
    ...
    try {
      ...
      if (stored) {
        ...
        if (typeof parsed === 'object' && parsed !== null) {
          setAnswers(parsed);
          // Restore profile image state
          if (parsed.profile_image) {
            setUploadedImageUrl(parsed.profile_image);
            setUploadedImageName('Already Uploaded Photo.jpg');
          }
          // Restore pitch video state
          if (parsed.pitch_video) {
            setUploadedVideoUrl(parsed.pitch_video);
            setUploadedVideoName('Already Uploaded Video.mp4');
          }
        }
      }
    } catch (e) {
  ...
  const validateCurrentStep = (): boolean => {
    ...
    for (const field of currentStep.fields) {
      ...
      if (field.required && !value) {
        if (field.type === 'image') {
          setValError('Please upload your Profile Photo.');
          return false;
        }
  ...
  // Secure File Upload Process mapping to Cloudinary upload
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fileType: 'image' | 'video', fieldId: string) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;
    const file = files[0];

    // Check size limitations (10MB for images, 50MB for video matches PRD)
    const MAX_SIZE_MB = fileType === 'image' ? 10 : 50;
    if (file.size > MAX_SIZE_MB * 1024 * 1024) {
      setValError(`${fileType === 'image' ? 'Photo' : 'Video'} size exceeds the ${MAX_SIZE_MB}MB limit.`);
      return;
    }
    ...
    try {
      ...
      updateAnswer(fieldId, uploadResult.url);
    } catch (err: any) {
      setValError(err?.message || `Failed to upload your ${fileType === 'image' ? 'photo' : 'video'}.`);
    } finally {
      if (fileType === 'image') {
        ...
        setTimeout(() => setVideoUploadProgress(null), 1000);
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!uploadedImageUrl) {
      setValError('Profile Photo is mandatory. Please upload a photo.');
      return;
    }
    if (videoUploadVisible && videoRequired && !uploadedVideoUrl) {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.message || 'Verification initialization gateway refused interaction.');
      }

      // Clear draft progress safely on navigation
      ...
      if (data.authorization_url) {
        ...
        throw new Error('Transaction initialization responded but did not provide authorization URL.');
      }
    } catch (err: any) {
      setValError(err?.message || 'Bill processing pipeline encountered error. Try again later.');
      setIsSubmitting(false);
    }
  };
                ...
                {field.type === 'image' && (
                  <div>
                    <input
                      type="file"
                      ref={imageInputRef}
                      onChange={(e) => handleFileChange(e, 'image', field.id)}
                    ...
                    {uploadedImageUrl ? (
                      <div className="border-[1.5px] border-green-500 p-5 rounded-3xl bg-green-500/5 flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          <div className="relative w-12 h-12 rounded-xl overflow-hidden border border-green-500/30 shrink-0">
                            <Image
                          ...
                          <div className="truncate text-left">
                            <span className="block text-xs font-black text-black">Profile Photo Saved</span>
                            <span className="block text-[10px] text-zinc-500 truncate">{uploadedImageName}</span>
                          </div>
                        </div>
                        <button
                          type="button"
                      ...
                      <div
                        ...
                        className={`border border-dashed border-[#C59B46] p-8 rounded-[2rem] flex flex-col items-center justify-center text-center bg-white/40 hover:bg-white cursor-pointer transition-colors shadow-sm group select-none ${isUploadingImage ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        ...
                        {isUploadingImage ? (
                          <div className="space-y-1">
                            <p className="text-xs font-black text-black uppercase">Uploading Photo to Cloudinary...</p>
                            {imageUploadProgress && (
                              <div className="w-40 h-1.5 bg-zinc-200 rounded-full overflow-hidden mx-auto">
                                <div className="h-full bg-[#C59B46] transition-all duration-300" style={{ width: `${imageUploadProgress}%` }} />
                              </div>
                            )}
                          </div>
                        ) : (
                          <>
                            <p className="text-xs font-black text-black uppercase tracking-wider mb-1">Upload Profile Photo</p>
                            <span className="text-[10px] text-zinc-500 uppercase font-medium">Accepts PNG, JPG up to 10MB</span>
                          </>
                        )}
                      </div>
                    )}
                ...
                {field.type === 'video' && (
                  <div>
                    <input
                      type="file"
                      ref={videoInputRef}
                      onChange={(e) => handleFileChange(e, 'video', field.id)}
                    ...
                    {uploadedVideoUrl ? (
                      <div className="border-[1.5px] border-green-500 p-5 rounded-3xl bg-green-500/5 flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="truncate text-left">
                            ...
                            <span className="block text-[10px] text-zinc-500 truncate">{uploadedVideoName}</span>
                          </div>
                        </div>
                        <button
                          type="button"
                      ...
                      <div
                        ...
                        className={`border border-dashed border-[#C59B46] p-8 rounded-[2rem] flex flex-col items-center justify-center text-center bg-white/40 hover:bg-white cursor-pointer transition-colors shadow-sm group select-none ${isUploadingVideo ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        <div className="w-12 h-12 bg-white border border-[#C59B46]/30 rounded-full flex items-center justify-center text-[#A67C00] mb-3 group-hover:scale-105 transition-transform shadow-md">
                          {isUploadingVideo ? (
                            <Loader2 className="animate-spin text-[#C59B46]" size={20} />
                          ) : (
                            <UploadCloud size={20} />
                          )}
                        </div>
                        ...
                        {isUploadingVideo ? (
                          <div className="space-y-1">
                            <p className="text-xs font-black text-black uppercase">Uploading Video to Cloudinary...</p>
                            {videoUploadProgress && (
                              <div className="w-40 h-1.5 bg-zinc-200 rounded-full overflow-hidden mx-auto">
                                <div className="h-full bg-[#C59B46] transition-all duration-300" style={{ width: `${videoUploadProgress}%` }} />
                              </div>
                            )}
                          </div>
                        ) : (
          ...
          {!isFinalStep ? (
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  {settings && Number(settings.registration_fee) === 0 ? (
                    <>Register for Free <ArrowRight size={14} /></>
                  ) : (
                    <>Register & Pay GHC {settings?.registration_fee ?? 0} <ArrowRight size={14} /></>
                  )}
                </>
              )}
            </button>
