...
export function Sidebar({ mobileOpen, setMobileOpen }: { mobileOpen?: boolean, setMobileOpen?: (val: boolean) => void }) {
  ...
  const navItems = [
    ...
    { label: 'Announcements Board', path: '/administrator/announcements' },
    { label: 'Gallery Showcase', path: '/administrator/gallery' },
    ...
    { label: 'Rules & Terms CMS', path: '/administrator/rules' },
  ];
    ...
    <>
      ...
      <aside className={cn(
        ...
        <nav className="flex flex-col gap-2">
          {navItems.map((item) => {
            const isActive = pathname === item.path || (item.path === '/administrator' && pathname === '/administrator');
            return (
              <Link
                key={item.label}
                href={item.path}
        ...
        <div className="mt-auto pt-6 border-t border-white/10 space-y-3">
           ...
           <Link href="/" className="text-sm opacity-70 px-3 py-2.5 rounded-md hover:bg-white/5 hover:opacity-100 flex items-center gap-3 font-medium">
              &larr; Back to Site
           </Link>
           <button 
             onClick={async () => {
