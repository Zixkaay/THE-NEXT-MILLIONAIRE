  ...
  return (
    <div className="animate-in fade-in duration-500 pb-24 bg-bg">
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-16 relative z-10 space-y-16">
        ...
        {/* Top Post: "Front Line" */}
        {featured && (
          <Link to={`/blog/${featured.id}`} className="group block bg-card border border-border p-2 rounded-3xl shadow-2xl overflow-hidden hover:border-accent transition-colors">
        ...
        <div className="pt-8">
          ...
          <div className="space-y-6 sm:space-y-8">
            {others.map(post => (
              <Link to={`/blog/${post.id}`} key={post.id} className="group flex flex-col sm:flex-row gap-4 sm:gap-8 items-center bg-card rounded-2xl overflow-hidden border border-border p-4 hover:border-accent transition-colors">
