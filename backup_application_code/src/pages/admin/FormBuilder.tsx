...
export function AdminFormBuilder() {
  ...
  const fetchFormConfig = async () => {
    ...
    try {
      ...
      setSteps(DEFAULT_FORM_STEPS);
      setPreviewStep(DEFAULT_FORM_STEPS[0].number);
    } catch (err: any) {
      setFeedback({ type: 'error', message: 'Failed loading form configuration.' });
    } finally {
      setLoading(false);
    }
  };
...
// ... (previous code)
  ...
  const handleSave = async (updatedSteps = steps) => {
    ...
    setFeedback(null);
    try {
      // Mock saving: In a pure FE app, this doesn't persist
      console.log('Mocking save of steps:', updatedSteps);
      setFeedback({ type: 'success', message: 'Form Builder configuration "saved" (Demo mode - no backend).' });
      setSteps(updatedSteps);
    } catch (err: any) {
      setFeedback({ type: 'error', message: 'Transaction portal connection lost.' });
    } finally {
      setSaving(false);
    }
  };

  // Re-initialize to default baseline seeds
  const handleResetToDefault = async () => {
    if (!window.confirm('Are you sure you want to reset all dynamic fields to default system seed configurations? This will erase custom additions.')) {
      return;
    }
    ...
    await handleSave(DEFAULT_FORM_STEPS);
  };
  ...
  const handleDeleteField = (stepNum: number, fId: string) => {
    if (!window.confirm('Delete this field? Raw submissions containing this field identifier will not be deleted but won\'t collect new answers.')) {
      return;
    }
  ...
  const handleFieldSubmit = (e: React.FormEvent) => {
    ...
    // Build unique ID if adding or reuse editing
  ...
  const handleDeleteStep = (stepNum: number) => {
    if (!window.confirm('Erase this entire multi-step column and all its assigned input fields permanently from drafts?')) {
      return;
    }
    const filtered = steps.filter(s => s.number !== stepNum);
    // Reorder step numbers from 1..
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 shrink-0 self-start sm:self-center">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-3 bg-[#101622] border border-[#232B3A] text-[#A3A3A3] hover:text-white rounded-xl text-xs font-black uppercase tracking-widest transition-colors"
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-5 py-3 bg-accent hover:opacity-90 disabled:opacity-50 text-black font-black uppercase text-xs tracking-widest rounded-xl transition-opacity shadow-[0_4px_20px_rgba(212,175,55,0.25)]"
          >
      ...
      {feedback && (
        <div className={`p-4 rounded-xl border flex items-center gap-3 animate-in slide-in-from-top-4 duration-300 ${feedback.type === 'success' ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
          <AlertCircle size={18} />
      ...
      {loading ? (
        <div className="p-12 text-center bg-card border border-border rounded-2xl">
          <RefreshCw size={32} className="animate-spin text-accent mx-auto mb-4" />
          ...
          <div className="lg:col-span-12 xl:col-span-8 space-y-6">
            ...
            {steps.length === 0 ? (
              <div className="p-8 text-center bg-card border-2 border-dashed border-border rounded-2xl">
                <Layers size={36} className="text-[#A3A3A3] mx-auto mb-3" />
                ...
                <button 
                  ...
                  className="px-4 py-2.5 bg-accent text-black text-xs font-extrabold uppercase tracking-wider rounded-xl hover:opacity-90 transition-opacity"
                >
                  Create First Step
                </button>
              </div>
                ...
                <div key={step.number} className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
                  ...
                  <div className="bg-[#101622] px-6 py-4 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-lg bg-accent/10 border border-accent/20 flex items-center justify-center text-accent font-black text-xs font-mono">
                        {step.number}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="px-3 py-1.5 bg-accent/10 border border-accent/20 hover:bg-accent hover:text-black hover:border-transparent rounded-lg text-accent text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1.5"
                      >
                        <Plus size={12} className="stroke-[3px]" /> Add Field
                      </button>
                      <button 
                        onClick={() => handleDeleteStep(step.number)}
                        className="p-1 px-2 hover:bg-red-500/15 text-red-500/80 hover:text-red-400 border border-transparent hover:border-red-500/10 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors"
                      >
                        Delete Step
                      </button>
                    </div>
                  ...
                  <div className="p-4 sm:p-6 divide-y divide-[#232B3A]/40">
                    {step.fields.length === 0 ? (
                      ...
                      step.fields.map((field, index) => (
                        <div key={field.id} className="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                          
                          {/* Field Information Block */}
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-white font-bold text-sm tracking-wide">{field.label}</span>
                              <span className="text-[9px] font-mono uppercase bg-[#141B26] text-text-sub border border-[#232B3A] px-1.5 py-0.5 rounded font-black tracking-widest">
                                ID: {field.id}
                              </span>
                              ...
                              {field.required && (
                                <span className="text-[9px] font-sans font-black uppercase bg-red-500/10 text-red-400 px-1.5 py-0.5 rounded tracking-widest">
                                  REQUIRED
                                </span>
                              )}
                            </div>
                            ...
                            {field.options && field.options.length > 0 && (
                              <div className="flex items-center gap-1 mt-1">
                                <span className="text-[10px] text-text-sub font-mono uppercase font-black">Dropdown Selection:</span>
                          ...
                          <div className="flex items-center gap-1 shrink-0 self-end sm:self-center">
                            ...
                            <button 
                              ...
                              disabled={index === 0}
                              className="p-2 text-[#A3A3A3] hover:text-white hover:bg-background/40 rounded-lg disabled:opacity-20 transition-all border border-transparent hover:border-[#232B3A]"
                              title="Move field up"
                            >
                              <ArrowUp size={14} />
                            </button>
                            ...
                            <button 
                              ...
                              className="p-2 text-[#A3A3A3] hover:text-[#d4af37] border border-transparent hover:border-[#d4af37]/10 hover:bg-[#d4af37]/5 rounded-lg transition-all ml-1"
                            ...
                            <button 
                              ...
                              className="p-2 text-[#A3A3A3] hover:text-red-500 border border-transparent hover:border-red-500/10 hover:bg-red-500/5 rounded-lg transition-all"
            ...
            <div className="pt-2">
              <button 
                onClick={handleAddStep}
                className="w-full py-4 border-2 border-dashed border-[#232B3A] hover:border-accent hover:bg-accent/5 rounded-2xl flex items-center justify-center gap-2 text-[#A3A3A3] hover:text-white text-xs font-black uppercase tracking-widest transition-all"
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-[#232B3A]/30">
                    {steps.map(s => (
                      <button 
                        ...
                        className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors ${previewStep === s.number ? 'bg-accent text-black font-extrabold' : 'bg-[#101622] text-[#A3A3A3] border border-[#232B3A]/40 hover:text-white'}`}
                      >
                        {s.title}
                      </button>
                    ))}
                  ...
                  <div className="bg-[#101622] p-5 border border-[#232B3A]/40 rounded-xl space-y-4">
                      ...
                      {steps.find(s => s.number === previewStep)?.fields.map(field => (
                        <div key={field.id} className="space-y-1.5 text-left">
                          ...
                          {field.type === 'textarea' ? (
                            ...
                            <div className="bg-background border border-[#232B3A] cursor-not-allowed border-dashed rounded-xl p-4 text-center text-text-sub text-xs opacity-75">
                              [ Dynamic Video Upload Criteria Input ]
                            </div>
                          ) : (
                            <input 
                              type={field.type} 
                    ...
                    <div className="pt-2 border-t border-[#232B3A]/40 flex justify-end">
                      <button disabled className="px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-[#A67C00] text-black font-black text-[10px] uppercase tracking-widest rounded-lg opacity-40 cursor-not-allowed">
          ...
          <div className="bg-[#101622] rounded-3xl border border-[#232B3A] w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 shadow-2xl">
            ...
            <div className="px-6 py-4 bg-[#141B26] border-b border-[#232B3A]/40 flex items-center justify-between">
              ...
              <button 
                ...
                className="text-[#A3A3A3] hover:text-white bg-transparent border-0 outline-none"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleFieldSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    if (modalMode === 'add') {
                      // Autopopulate id to be clean database format
                      const slug = e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '_');
                      setFieldId(slug);
                    }
                  }}
                  className="w-full bg-[#141B26] border border-[#232B3A] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-accent"
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-[#141B26] border border-[#232B3A] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-accent disabled:opacity-50 disabled:cursor-not-allowed font-mono"
              ...
              {fieldType === 'select' && (
                <div className="animate-in slide-in-from-top-1">
                  ...
                  />
                  <p className="text-[9px] text-[#A3A3A3] italic mt-1 font-sans">
              ...
              <div className="flex items-center justify-between p-4 bg-background/50 border border-[#232B3A]/40 rounded-xl">
                ...
                <button 
                  ...
                  className="p-1 text-[#d4af37] bg-transparent hover:bg-white/5 rounded-lg border border-transparent"
                >
                  {fieldRequired ? (
              ...
              <div className="pt-4 border-t border-[#232B3A]/40 flex justify-end gap-2">
                <button 
                  type="button"
                  onClick={() => setShowFieldModal(false)}
                  className="px-4 py-2.5 bg-[#141B26] border border-[#232B3A] text-[#A3A3A3] hover:text-white rounded-xl text-xs font-black uppercase tracking-widest"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  className="px-5 py-2.5 bg-accent text-black font-black uppercase text-xs tracking-widest rounded-xl transition-opacity hover:opacity-90 flex items-center gap-1 shadow-md"
                >
